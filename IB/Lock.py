...
class Lock (object):
  def __init__ (self, dirname, checkStale=False, stableGap=600):
    self.piddir   = dirname
    self.pidfile  = join(self.piddir,"pid")
    self.pid      = str(getpid())
    self.locktime = 0
    self._hasLock = self._get()
    if not self._hasLock and self.locktime and checkStale and (time()-self.locktime)>=stableGap:
      self._release(True)
      self._hasLock = self._get()
  ...
  def _release (self, force=False):
    if not force and self._hasLock and self._get(): force = True
    if force: getstatusoutput ("rm -rf %s" % self.piddir)
    self.locktime = 0
    self._hasLock = False
