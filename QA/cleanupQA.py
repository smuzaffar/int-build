...
def cleanArea(dirType, daysToKeep=None, dryRun=False, sudo=False):
    cmdargs =  " /afs/cern.ch/user/c/cmsbuild/ib-scheduler/cleanupDirs.py --type "+dirType
    if sudo:       cmdargs += " --sudo "
    if dryRun:     cmdargs += " --dryRun "
    if daysToKeep: cmdargs += " --days "+str(daysToKeep)
    thrds = []
    import commands
    boxes=commands.getoutput("acrontab -l | grep -v '^ *#' | grep '/runQA.py' | awk '{print $6}' | sort | uniq").split('\n')
    for box in boxes:
        box = box.strip()
        cmd = "ssh "+box+cmdargs
        ...
        thrds.append(thd)
        thd.start()
    thd = CleanupThread(cmdargs); thrds.append(thd); thd.start()
    hasActive = True
    maxTime = 60*30
    while (hasActive):
        time.sleep(60)
